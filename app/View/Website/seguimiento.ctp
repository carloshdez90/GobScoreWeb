...
<script>
	$(document).on('ready', function(){
		...
		var denuncia_id = '<?php echo $denuncia_id; ?>';
		...
		$('.basic').on('click', function() {
			$.ajax({
				'type' : 'post',
				'url'  : 'http://'+servidor+'/website/calificar',
				'data' : {
					'denuncia_id' : denuncia_id,
					'calificacion' : $('.rate-value').val(),
				},
				'success' : function (data) {
					if (data.message) {
						return alert('Se ha calificado esta denuncia con éxito.')
					}
